...
use Shetabit\Chunky\Contracts\OutputStreamInterface;

class Http implements OutputStreamInterface
{
    protected $config;
    protected $request;
    ...
    {
        abort_unless(is_file($path), 400, 'Invalid Request');

        $this->request = $request;
        $this->size = filesize($path);
        $this->mime = mime_content_type($path);
        $this->file = fopen($path, "r");
        $this->boundary = md5($path);
        ...
        if (
            ...
            $this->request->hasHeader('HTTP_RANGE') &&
